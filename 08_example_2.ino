...
// turn on the LED if the distance is between dist_min and dist_max
  ...
  if(dist_raw > dist_min && dist_raw <= 200){
    analogWrite(PIN_LED, (100-dist_raw)*2.55);
  }
  else if(dist_raw > 200 && dist_raw < dist_max) {
    analogWrite(PIN_LED, (dist_raw-200)*2.55);
  }
  else {
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading > dist_min && reading < dist_max)
    save_read=reading;
  if(reading < dist_min || reading > dist_max)
    reading = save_read; // return 0 when out of range.
  return reading;
